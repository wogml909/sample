...
public class CompareValue {
	...
	public static void main(String[] args) {
		...
		// Q. intObj 객체와 intObj2 의 값이 같은가?
		// A. 10이라는 값은 같다
		// 값을 꺼내서 비교(언박싱하여 비교)
		System.out.println("언박싱하여 비교"); // 값
		System.out.println("결과 : " + (intObj1.intValue() == intObj2.intValue()));

		// Q. intObj 객체와 intObj2 의 인스턴스가 같은가?
		// A. byte 범위에서는 같은 레퍼런스(인스턴스)로 생성된다.
		// *byte(8bit) -128~127 까지 범위내 숫자를 사용하면 별도의 인스턴스를 생성하지않는다.
		// *byte(8bit) -128~127 까지 범위에서는 같은 인스턴스로 생성한다.
		System.out.println("레퍼런스로 비교"); // 주소
		System.out.println("결과 : " + (intObj1 == intObj2));

		System.out.println("equals()로 비교 *인스턴스로 비교"); // 주소끼리 같은지
		...
		System.out.println("레퍼런스로 비교"); // 주소 객체 메모리공간 같을 수없다 byte(8bit) -128~127 까지 범위
