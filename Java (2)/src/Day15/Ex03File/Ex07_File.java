...
public class Ex07_File {
	...
	/*
	 * 디렉토리 파일 목록 출력하는 메소드 filePath
	 **/
	private static void listDirectory(String filePath) {
		System.out.println(filePath);
		System.out.println("------------------------------------------");
		File file = new File(filePath);
		File[] fileList = file.listFiles(); // 하위 경로의 파일목록을 File 배열로 변환
	...
	private static void deleteDirectory(String filePath) {
		...
		if (fileList != null) {
			...
			for (File subFile : fileList) {
				// 재귀 호출
				deleteDirectory(subFile.getPath()); // 하위 파일의 경로
			}
		}
		// 폴더(파일) 삭제
		if (file.delete()) {
