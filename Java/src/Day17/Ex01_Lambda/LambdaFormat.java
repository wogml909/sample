...
@FunctionalInterface // 컴파일러에 해당하는 인터페이스가 함수형 인터페이스임을 명시적으로 알려준다.
interface FuncInterface1{ // 1)함수형인터페이스 정의
	 //@FunctionalInterface 를 지정하면, 추상메소드를 2개이상 정의하면 에러를 발생하게 된다.
	void method();
...
public class LambdaFormat {
	...
	public static void main(String[] args) {
		...
		FuncInterface1 fi1 = () -> System.out.println("자바람다식1"); // 2)람다식인터페이스 정의
        fi1.method();//3)정의
		...
		// 매개변수, 반환타입 있는 람다식
		// (a, b) -> {return a + b ;}
		// (a, b) -> return a + b ; *에러 : return이 있을땐, {} 생략불가
		// (a, b) -> a + b ; *return 문만 있다면, "return,{}" 동시생략가능
         //(에러)FuncInterface4 fi4 = (a, b) -> return a + b;;
        FuncInterface4 fi4 = (a, b) ->  a + b;
