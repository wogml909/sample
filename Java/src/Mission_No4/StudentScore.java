...
public class StudentScore {
		...
		for (int i = 0; i < classStudentScore.length; i++) {
			if (classYn[i]) {
				...
				for (int j = 0; j < classTotalAverage[i].length; j++) {
					if (j == classTotalAverage[i].length - 1)
						System.out.print("\t");
	...
	public static void classList() {
		...
		for (int i = 0; i < classStudentScore[classNo - 1].length; i++) {
			if (classYn[classNo - 1] && studentYn[classNo - 1][i]) {
				...
				System.out
						.print(classStudentData[classNo - 1][i][0] + "\t" + classStudentData[classNo - 1][i][1] + "\t");
				System.out
						.print(classStudentRank[classNo - 1][i][0] + "\t" + classStudentRank[classNo - 1][i][1] + "\t");
				System.out.println();
			}
		}
		System.out.println(
	...
	public static void searchStudent() {
		...
		if (classYn[classNo - 1] && studentYn[classNo - 1][studentNo - 1]) {
			...
			System.out.print(classStudentData[classNo - 1][studentNo - 1][0] + "\t"
					+ classStudentData[classNo - 1][studentNo - 1][1] + "\t");
			System.out.print(classStudentRank[classNo - 1][studentNo - 1][0] + "\t"
					+ classStudentRank[classNo - 1][studentNo - 1][1] + "\t");
			System.out.println();
		}
	...
	public static void calcClassTotal(int classNo) {
		...
		// 반 전체 평균
		classTotalAverage[classNo - 1][6] = Math
				.round((double) classTotalScore[classNo - 1][6] / classCount[classNo - 1] / 6 * 100.0) / 100.0;
