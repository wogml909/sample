...
//데이터 목록 
	public List<Board> list(){
		...
		String sql = " SELECT * "
				+ " FROM board ";
		try {
		//쿼리(sql) 실행 객체 생성 -Statement (stmt) stmt 변수로 선언
			stmt = con.createStatement();
			
		//쿼리(sql) 실행-> 결과  -ResultSet (rs) rs 변수로 선언
...
//데이터 조회 : 게시글 목록 번호를 불러온다
	public Board select(int no){
		...
		String sql = "SELECT * "
				...
				+ " WHERE no = ? ";
		try {
		//쿼리(sql) 실행 객체 생성 -PreparedStatment(psmt)
...
//데이터 등록
	public int insert(Board board) {
		...
		try {
			...
			result = psmt.executeUpdate();      //SQL 실행요청, 적용된 데이터 개수를 받아온다
                                                //게시글 1개 적용 성공 시, result : 1
			                                    //          실폐 시, result : 0
			//executeUpdate()
			//:SQL(INSERT,UPDATE.DELETE)을 실행하고 적용된 데이터 개수를 int타입으로 변환
		} catch (SQLException e) {
